...
MainWindow::MainWindow(QWidget *parent)
    ...
    , m_neuralNetwork(new NeuralNetwork(784, 256, 128, 10))
    , m_datasetLoader(new DatasetLoader())
    , m_cellProcessor(new CellProcessor())
{
    setupUI();
    if (m_neuralNetwork->loadModel("autosave_model.dat")) {
        logMessage("Модель загружена из model1.dat");
...
void MainWindow::recognizeCells()
{
    ...
    if (m_gridResult.cells.isEmpty()) {
        logMessage("Автоматическое детектирование не сработало. Используем ручную сетку.");
        addManualGrid();
        return;
    }
...
void MainWindow::performRecognition()
{
    ...
    for (int i = 0; i < m_gridResult.cells.size(); i++) {
        ...
        int digit = m_neuralNetwork->predict(input);
        QVector<double> outputs = m_neuralNetwork->getOutputs();
        double confidence = outputs[digit];

        RecognizedCell cell;
        cell.character = QChar('0' + digit);
...
void MainWindow::updateResultsTable()
{
    ...
    for (int i = 0; i < m_recognizedCells.size(); i++) {
        ...
        QTableWidgetItem *digitItem = new QTableWidgetItem(QString(cell.character));
        digitItem->setTextAlignment(Qt::AlignCenter);
        if (cell.confidence < 0.7) {
            digitItem->setBackground(QBrush(QColor(255, 200, 200)));        }
        ...
        correctionEdit->setPlaceholderText("0-9");
        ...
        if (cell.corrected) {
            ...
            correctionEdit->setText(cell.userCorrectedDigit);
        }

        m_resultsTable->setItem(i, 0, cellItem);
        m_resultsTable->setItem(i, 1, digitItem);
...
void MainWindow::applyCorrection(int row)
{
    ...
    if (correction.isEmpty()) {
        QMessageBox::information(this, "Информация", "Введите цифру от 0 до 9");
        return;
    }

    bool ok;
    int digit = correction.toInt(&ok);
    if (!ok || digit < 0 || digit > 9) {
        QMessageBox::warning(this, "Ошибка", "Введите цифру от 0 до 9");
        return;
    }
    m_recognizedCells[row].character = QChar('0' + digit);
    m_recognizedCells[row].corrected = true;
    m_recognizedCells[row].userCorrectedDigit = correction;
    ...
    logMessage(QString("Клетка %1 исправлена: %2 -> %3")
                   .arg(row + 1)
                   .arg(m_recognizedCells[row].character)
                   .arg(correction));
}

void MainWindow::saveCorrectedSample(const QImage& cellImage, const QString& correctDigit)
{
    QImage processed = m_cellProcessor->getProcessedImage(cellImage);
    QVector<double> input = m_cellProcessor->preprocessCell(cellImage);
    QVector<double> target(10, 0.0);
    int digit = correctDigit.toInt();
    target[digit] = 1.0;

    TrainingData data;
    data.input = input;
    data.target = target;
    data.label = correctDigit;

    m_corrections.append(data);

    logMessage(QString("Добавлено исправление для цифры %1").arg(correctDigit));
}
...
void MainWindow::addManualGrid()
{
    ...
    QImage cleanedImage = m_gridDetector->removeBlueLines(m_originalImage);
    ...
    logMessage("Синие линии удалены, остались только черные цифры");
    performRecognition();
}
...
{
    m_neuralNetwork->saveModel("autosave_model.dat");
